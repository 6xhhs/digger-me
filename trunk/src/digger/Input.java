...
public boolean isUpPressedFromClear() {
    return upPressedFromClear;
}
public void clearUpPressedFromClear() {
    upPressedFromClear = false;
}

boolean downPressedFromClear = false;
public boolean isDownPressedFromClear() {
    return downPressedFromClear;
}
public void clearDownPressedFromClear() {
    downPressedFromClear = false;
}

boolean firePressedFromClear = false;
public boolean isFirePressedFromClear() {
    return firePressedFromClear;
}
public void clearFirePressedFromClear() {
    firePressedFromClear = false;
}


Input (DiggerCanvas d) {
	dig = d;
}
void checkkeyb () {
	if (pluspressed) {
		if (dig.frametime>DiggerCanvas.MIN_RATE)
			dig.frametime -= 5;
	}
	if (minuspressed) {
		if (dig.frametime<DiggerCanvas.MAX_RATE)
			dig.frametime += 5;
	}
	if (f10pressed)
		escape=true;
